...
func InitializeStatusUpdates(bcast_status_update chan elevator.Elev, receive_status_update chan elevator.Elev) {
  go bcast.Transmitter(PORT_STATUS_UPDATES, bcast_status_update)
  go bcast.Receiver(PORT_STATUS_UPDATES, receive_status_update)
}


func InitializeOrders(bcast_order chan orderDistributerHF.ExtOrder, receive_ext_order chan orderDistributerHF.ExtOrder) {
  go bcast.Transmitter(PORT_ORDERS, bcast_order)
  go bcast.Receiver(PORT_ORDERS, receive_ext_order)
}


func InitializeOrderUpdates(bcast_order_executed chan int, receive_order_executed chan int) {
